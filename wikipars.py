    ...
    for name_rep in list_repos:
        ...
        for commit in wiki_commits:
            ...
            if commit.author.email and len(commit.author.email.split('+')) > 1:
                info[AUTHOR_LOGIN] = commit.author.email.split('+')[1].split('@users')[0]
            if parent:
                info[PAGE] = ';'.join([diff.b_path for diff in parent[0].diff(commit)])
                info[ACTION] = ';'.join([ACTIVITY[diff.change_type] for diff in parent[0].diff(commit)])
            else:
                # Первый коммит
                info[PAGE] = ';'.join([diff.b_path for diff in commit.diff(EMPTY_TREE_SHA)])
                info[ACTION] = ';'.join([ACTIVITY["A"]])
            for fieldname in info:
                print(fieldname, info[fieldname], sep=': ')
            print("-" * 40)
            log_wiki_to_csv(info, csv_name)
            data_changes.append(info)
